...
for i_seq, seq in tqdm(enumerate(ds_test.sequences), total=len(ds_test.sequences)):
    npypath = ds_test.get_numpy_paths_for_sequence(seq)
    npy = np.load(npypath)
    ...
    plt.savefig(os.path.join(PRED_DIR, f"{i_seq}.png"))
    plt.show()
